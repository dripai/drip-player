...
影子播放器是一个基于 Tauri 2、Vue 3 和 Rust 的桌面媒体播放器，面向本地课程视频、音频资料和在线视频链接管理。它扫描设置中的媒体目录，下载 YouTube、Bilibili 等在线媒体，并通过媒体探测选择合适的播放方式。
...
- 下载窗口：解析 URL 后选择实际可用的清晰度，再开始下载；解析和下载使用同一个浏览器登录来源，只有主动选择“仅音频 · MP3”才转换为音频。关闭窗口继续下载，文件通过类型、尺寸和时长校验后加入列表，不打断播放。详见[下载与清晰度选择](docs/downloads.md)。
- 下载恢复：任务及格式选择保存到 SQLite；待选择任务保留选项，退出时运行中的任务下次显示“已中断”。每次继续使用当前设置目录，所选格式失效时要求重新选择；切换目录会停止正在运行的下载，旧文件保留原位。
...
- 右键菜单：页面和播放列表共用跟随主题的菜单；页面保留刷新、设置，列表项支持重命名、移除及确认后永久删除媒体文件。详见[播放列表文件操作](docs/playlist-files.md)。
...
1. MP4 中的 H.264、HEVC、AV1 等浏览器可处理的容器与编码进入内置视频播放器。HEVC 的实际解码依赖 WebView 和系统能力，失败时明确提示，不强制启动 MPV；[Windows 条件](https://learn.microsoft.com/en-us/troubleshoot/microsoft-edge/development/video-playback-issues)。
2. 如果可以无损转封装成浏览器兼容 MP4，则写入本地 remux 缓存后播放。
3. 如果是纯音频，交给 Rust 音频后端播放。
4. 如果是视频但浏览器链路无法处理，并且随包 `lib/` 中包含 MPV，则使用 MPV 播放。

探测工具缺失或文件探测失败直接报错，不按扩展名改用其他引擎。新下载在同分辨率下优先选择 H.264/AAC，成品直接保存在设置目录中；同名文件和字幕统一加序号，详见[下载说明](docs/downloads.md)。
