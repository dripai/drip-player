...
- SQLite 本地存储：播放列表、媒体资料、外观、界面语言、播放模式和关闭到托盘设置。
...
数据库位于可执行文件旁的 `config/drip-player.sqlite3`，首次启动使用空播放列表和默认设置。旧 JSON 和浏览器设置不导入，旧文件不会被删除。移除播放列表条目保留媒体资料与文件。表结构与读写规则见 [SQLite 存储说明](docs/sqlite-storage.md)。
