...
| `download_jobs` | 下载任务 JSON，包括 URL、阶段、登录来源、格式选项与选择、预期时长、尝试次数、目录记录、完成路径、错误；由 `download_schema.sql` 创建 |
...
- 下载先写入 `<当前目录>/downloading/<job_id>`。校验完成后，先登记每个媒体、字幕文件的源名称、目标名称和文件版本，再以禁止覆盖的方式逐个移动到 `<当前目录>/<文件名>`；同名时为媒体和字幕统一追加序号。资源、列表成员和任务完成状态在同一事务中提交，同时清空发布日志。失败时在释放目录锁前恢复临时文件；启动时恢复部分移动，文件已修改或回滚失败则明确报错。方案标记留在临时区，不进入成品目录。
- 每次重试读取当前目录设置，历史目录仅用于展示和恢复未提交的发布操作。目录切换先停止下载进程；同目录继续可复用临时文件，换目录后重新下载。旧尝试不能覆盖新尝试结果。
- 解析完成后进入等待选择，选项写入 SQLite；用户确认后重新核对可用格式，再按当前目录下载。选择失效时返回等待选择，不降档。文件通过 ffprobe 类型、音轨、尺寸与时长校验后，才进入发布事务并更新媒体标题与类型。详见[下载说明](downloads.md)。
- 正常退出时停止活动任务；下次启动把运行中的任务标记为“已中断”，待选择任务保留选项，不自动下载。发布中崩溃时根据日志将尚未提交的文件移回临时目录，再等待手动继续。
